...
public class PlayerController2D : MonoBehaviour
{
    ...
    [HideInInspector] public float externalMoveInput;

    [HideInInspector] public bool externalJump;
    [HideInInspector] public bool externalJumpHeld;

    [HideInInspector] public bool externalDash;

    [HideInInspector] public bool externalShoot;
    [HideInInspector] public bool externalShootHeld;

    public bool useBotInput = false;
    ...
    void ApplyMovement()
    {
        ...
        rb.linearVelocity = new Vector2(
            Mathf.Lerp(rb.linearVelocity.x, targetSpeed, 12f * Time.fixedDeltaTime),
            rb.linearVelocity.y);
    }
    ...
    void HandleJumpHold()
    {
        bool holdingJump = useBotInput
            ? botJumpHeldThisPhysicsStep
            : Input.GetKey(jumpKey);

        if (holdingJump && rb.linearVelocity.y > 0)
        {
            rb.linearVelocity +=
                Vector2.up * jumpHoldBoost * Time.fixedDeltaTime;
        }
    }
    ...
    void ReduceStickiness()
    {
        ...
        RaycastHit2D leftHit = Physics2D.Raycast(
            transform.position, Vector2.left, 0.4f, groundLayer);
    ...
    void Flip()
    {
        if (moveInput > 0)
        {
            ...
            if (weaponHolder != null)
            {
                Vector3 p = weaponHolderDefaultLocalPos;
                p.x = Mathf.Abs(p.x);
                weaponHolder.localPosition = p;
            }
        }
        else if (moveInput < 0)
        {
